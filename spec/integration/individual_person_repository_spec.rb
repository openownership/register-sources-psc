...
require 'register_sources_psc/repositories/individual_person_repository'
require 'register_sources_psc/services/es_index_creator'
require 'register_sources_psc/structs/individual'

RSpec.describe RegisterSourcesPsc::Repositories::IndividualPersonRepository do
  ...
  let(:individual_person) do
    content = File.read('spec/fixtures/psc_individual.json')
    RegisterSourcesPsc::Individual.new(
  ...
  before do
    index_creator = RegisterSourcesPsc::Services::EsIndexCreator.new(
      individual_person_index: index,
      client: es_client
    )
    index_creator.create_individual_person_index
  end
