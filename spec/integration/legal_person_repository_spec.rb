...
require 'register_sources_psc/repositories/legal_person_repository'
require 'register_sources_psc/services/es_index_creator'
require 'register_sources_psc/structs/legal_person'

RSpec.describe RegisterSourcesPsc::Repositories::LegalPersonRepository do
  ...
  let(:legal_person) do
    content = File.read('spec/fixtures/psc_legal.json')
    RegisterSourcesPsc::LegalPerson.new(
  ...
  before do
    index_creator = RegisterSourcesPsc::Services::EsIndexCreator.new(
      legal_person_index: index,
      client: es_client
    )
    index_creator.create_legal_person_index
  end
